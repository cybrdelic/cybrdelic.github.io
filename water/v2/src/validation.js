...
export function validateGPU(engine) {
  ...
  if (reports.zeroAlphaImpulse.maxHeight < 0.1)
    throw new Error(
      "Numerical impulses were suppressed by visual alpha blending",
    );
  ...
  for (let i = 0; i < data.length; i += 4)
    px += data[i + 1] * engine.patch.dx ** 2 * 1025 * engine.patch.depth;
