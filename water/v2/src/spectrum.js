  ...
  constructor(n, preset, seed = 91317) {
    ...
    this.bands = BANDS.map((band, index) => {
      const h0 = new Float32Array(n * n * 4),
        rng = mulberry32(seed + 5989 * index);
      let variance = 0;
      for (let z = 0; z < n; z++)
        for (let x = 0; x < n; x++) {
          const kx = ((x <= n / 2 ? x : x - n) * TAU) / band.length,
            kz = ((z <= n / 2 ? z : z - n) * TAU) / band.length;
          const k = Math.hypot(kx, kz),
            wl = TAU / (k || 1e-12),
            omega = dispersion(k, preset.depth),
            i = 4 * (z * n + x);
          h0[i + 2] = omega;
          h0[i + 3] = k;
          if (
          ...
          const theta = Math.atan2(kz, kx),
            spec = (period, gamma) => {
              const wp = TAU / period,
                ss = omega <= wp ? 0.07 : 0.09,
                r = Math.exp(-0.5 * ((omega - wp) / (ss * wp)) ** 2);
              return (
                G *
                G *
                omega ** -5 *
          ...
          const directional = (angle, power) =>
            Math.exp(power * (Math.cos(theta - angle) - 1));
          const power =
            (((spec(preset.period, 3.3) * directional(preset.direction, 12) +
              0.55 *
                spec(preset.period * 0.55, 1.35) *
                directional(preset.direction + 0.24, 3)) *
              groupVelocity(k, preset.depth)) /
              k) *
            (TAU / band.length) ** 2;
          h0[i] = gaussian(rng) * Math.sqrt(power * 0.25);
          h0[i + 1] = gaussian(rng) * Math.sqrt(power * 0.25);
          variance += power;
        }
      this.expectedVariance += variance;
      return { ...band, h0, variance };
    });
    const scale = preset.hs / 4 / Math.sqrt(this.expectedVariance || 1);
    for (const b of this.bands)
      for (let i = 0; i < b.h0.length; i += 4) {
        b.h0[i] *= scale;
        b.h0[i + 1] *= scale;
      }
