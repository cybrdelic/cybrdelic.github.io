...
export class PostProcessor {
 constructor(renderer,shared,width,height){
  this.renderer=renderer;this.a=target(width,height,{type:THREE.HalfFloatType,linear:true});this.b=target(width,height,{type:THREE.HalfFloatType,linear:true});this.valid=false;
  this.uniforms={...shared,uCurrent:U(null),uMotion:U(null),uDepth:U(null),uHistory:U(this.a.texture),uInvVP:U(new THREE.Matrix4()),uPrevVP:U(new THREE.Matrix4()),uResolution:U(new THREE.Vector2(width,height)),uHistoryValid:U(0),uUnderwater:U(0),uTAA:U(1),uDt:U(1/24)};
  this.resolve=new Pass(`precision highp float;in vec2 vUv;out vec4 fragColor;${environmentGLSL}
  ...
  this.display=new Pass(`precision highp float;in vec2 vUv;out vec4 fragColor;uniform sampler2D uInput;uniform float uExposure;uniform vec2 uResolution;
   float hash(vec2 p){return fract(sin(dot(p,vec2(127.1,311.7)))*43758.5453);}
   void main(){vec3 c=max(texture(uInput,vUv).rgb*uExposure,0.);c=clamp((c*(2.51*c+.03))/(c*(2.43*c+.59)+.14),0.,1.);c=pow(c,vec3(1./2.2));float vig=1.-.065*pow(length((vUv-.5)*vec2(1.1,1.)),1.5);c=c*vig+(hash(gl_FragCoord.xy)-.5)/1024.;fragColor=vec4(c,1.);}`,{uInput:U(this.b.texture),uExposure:shared.uExposure,uResolution:this.uniforms.uResolution});
 }
 reset(){this.valid=false;}
 resize(w,h){this.a.setSize(w,h);this.b.setSize(w,h);this.uniforms.uResolution.value.set(w,h);this.reset();}
 render(sceneRT,viewProjection,underwater,time){
  const u=this.uniforms;u.uCurrent.value=sceneRT.texture;u.uMotion.value=sceneRT.textures[1];u.uDt.value=this.lastTime===undefined?0:Math.max(0,time-this.lastTime);this.lastTime=time;u.uDepth.value=sceneRT.depthTexture;u.uHistory.value=this.a.texture;u.uInvVP.value.copy(viewProjection).invert();u.uHistoryValid.value=this.valid?1:0;u.uUnderwater.value=underwater;
  this.resolve.run(this.renderer,this.b);this.display.uniforms.uInput.value=this.b.texture;this.display.run(this.renderer,null);[this.a,this.b]=[this.b,this.a];u.uPrevVP.value.copy(viewProjection);this.valid=true;
 }
}
