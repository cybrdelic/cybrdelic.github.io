...
export class FloatingBody {
  ...
  step(dt, samples, patch, time) {
    ...
    if (this.boat) {
      ...
      const thrust = clamp(2400 + (5.1 - speed) * 1400, -1800, 5500);
      force.addScaledVector(forward, thrust);
      const desired = -0.08 - 0.42 * Math.sin(time * 0.095),
    ...
    localTorque.set(
      localTorque.x / this.inertia.x,
      localTorque.y / this.inertia.y,
      localTorque.z / this.inertia.z,
    );
...
export class Dynamics {
  ...
  sampleAndStep(dt, time, ocean, patch, particles, preset) {
    ...
    for (let i = emitStart; i < samples.length; i++) {
      const s = samples[i];
      if (
        s.jacobian < preset.foamThreshold + 0.13 &&
        this.rng() < clamp((preset.foamThreshold + 0.13 - s.jacobian) * 4, 0, 1)
      ) {
        const p = positions[i];
